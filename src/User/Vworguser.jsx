...
const Vworguser = () => {
    ...
    useEffect(()=>{
        let fetchdata=async()=>{
           let response=await axios.get('http://localhost:4000/user/vieworg')
...
<form class="max-w-md mx-auto">   
    ...
    <div class="relative">
        ...
        <input type="search" id="default-search" class="block w-full p-4 ps-10 text-sm text-black border border-orange-500 rounded-lg bg-[#FFEFBD] focus:ring-orange-500 focus:border-orange-500   dark:placeholder-gray-400   " placeholder="Search Organization,District" required />
            ...
            <tr class="bg-[#f8d2a0] border-b border-orange-600 hover:bg-[#f7b866d4] text-black font-semibold">
               ...
               <td class="px-6 py-4">
                ...
                <td class="px-6 py-4  hover:font-bold ">
                     <Link to={`/user/vieworgdtluser/${item._id}`}><button className='hover:underline '>View</button></Link>
