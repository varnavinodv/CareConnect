...
const Vwreviewadm = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://localhost:4000/admin/viewreview');
                    ...
                    <table className='w-full text-sm text-left rtl:text-right text-black dark:text-black'>
                        ...
                        <tbody>
                            {filteredData.map((item, index) => (
                                <tr key={index} className='bg-[#f8d2a0] border-b border-orange-600 hover:bg-[#f7b866d4] font-semibold border'>
                                    <td className='px-6 py-4'>{index + 1}</td>
                                    <td className='px-6 py-4'>{item.orphanage?.name}</td>
                                    <td className='px-6 py-4'>{item.organization?.name}</td>
                                    <td className='px-6 py-4'>{item.review?.review}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
